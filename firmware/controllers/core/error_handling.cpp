...
// Функция strlncpy ПОЛНОСТЬЮ УДАЛЕНА - она определена в efistring.cpp
...
void errorHandlerInit() {
  ...
  // see [https://github.com/rusefi/rusefi/wiki/Resilience](https://github.com/rusefi/rusefi/wiki/Resilience)
...
void chDbgPanic3(const char *msg, const char * file, int line) {
  ...
  auto err = &bkpram->err;
  if (err->Cookie == ErrorCookie::None) {
    // ИСПРАВЛЕНО: заменить strlncpy на strncpy
    strncpy(err->file, file, efi::size(err->file) - 1);
    err->file[efi::size(err->file) - 1] = '\0';
    err->line = line;
    strncpy(err->msg, msg, efi::size(err->msg) - 1);
    err->msg[efi::size(err->msg) - 1] = '\0';
...
static void firmwareErrorV(ObdCode code, const char *fmt, va_list ap) {
  ...
  if (indexOf(fmt, '%') == -1) {
    ...
    criticalErrorMessageBuffer[sizeof(criticalErrorMessageBuffer) - 1] = '\0';
  } else {
